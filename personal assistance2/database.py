...
def turn_on_speech():
    if (check_internet_connection):
        ...
        cur.execute(query)
        con.commit()
        return ("speech turn on")
    else:
        return ("please turn on the internet first")
def turn_off_speech():
    con = create_connection()
    cur = con.cursor()
...
def mic_is_on():
    con = create_connection()
    cur = con.cursor()
    query = "select value from memory where name = 'mic_module'"
    cur.execute(query)
    ans = str(cur.fetchall()[0][0])
    if ans == "on":
        return True
...
def turn_on_mic():
    if (check_internet_connection):
        con = create_connection()
        cur = con.cursor()
        query = "update memory set value = 'on' where name = 'mic'"
        cur.execute(query)
        con.commit()
        return ("mic turned on")
    else:
        return ("please turn on the internet first")    
def turn_off_mic():
    con = create_connection()
    cur = con.cursor()
    query = "update memory set value = 'off' where name = 'mic'"
    cur.execute(query)
    con.commit()
    return ("mic is turned off")
